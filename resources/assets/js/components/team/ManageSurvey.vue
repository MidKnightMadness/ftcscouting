...
<script>
    export default {
        ...
        methods: {
            fetchSurveys(){
                axios.get('/api/team/' + this.number + '/surveys').then(response => {
                    this.surveys = response.data;
                });
            },
            archive(id){
                axios.post('/api/survey/' + id + '/archive', {archived: true}).then(r => this.fetchSurveys())
            },
            unarchive(id){
                axios.post('/api/survey/' + id + '/archive', {archived: false}).then(r => this.fetchSurveys())
            },
            deleteSurvey(survey){
            ...
            doDelete(id){
                ...
                axios.post('/api/survey/'+id+'/delete').then(r => this.fetchSurveys())
            }
        }
    }
</script>